 ...
 * own adiv5.c / cortexm.c / stm32f1.c code can enumerate and "attach" to a
 * target with no SWD pins driven. All constants below were derived directly
 * from the BMP source this integrates with (adiv5.c, cortexm.c, stm32f1.c),
 * not from datasheets, so the enumeration path matches byte-for-byte.
 *
 * The model is a pure word-addressed memory function. The shim (emu_shim.c)
 * turns adiv5 DP/AP transactions into calls on this model.
 */
#ifndef EMU_TARGET_H
#define EMU_TARGET_H
...
void emu_target_reset(void); /* re-seed flash/SRAM/debug state */
